...
- **MacroPedido**: unidad operativa del dashboard. Existe como máximo uno por `(sucursal_cliente, fecha_pedido)` y agrupa hasta **cinco** `Pedido` confirmados del mismo día local. Guarda total acumulado, estado común y `ultima_confirmacion`; la fecha visible es el día del macro y la hora visible es la del último hijo. La barra de cinco segmentos es sólo una representación del número de pedidos, con colores verde → amarillo → rojo. El límite se valida antes de capturar y de nuevo dentro de la transacción de confirmación, bloqueando la sucursal con `select_for_update()` para evitar que dos solicitudes concurrentes creen un sexto pedido.
- **Pedido**: pedido individual dentro del `MacroPedido`; conserva sus productos, total, clave y hora propios. Estados `pendiente → confirmado → enviado → recibido` (`recibido` queda para uso futuro/manual). El estado operativo se cambia sobre el macro y se sincroniza en todos sus hijos; un nuevo pedido agregado a un macro previamente enviado lo reactiva como confirmado. El admin puede deshacer exclusivamente `enviado → confirmado`; no hay exportación a Excel. Borrado desde la UI es lógico (`eliminado=True`). Solo puede existir **un pedido pendiente por sucursal** (`pedido_pendiente()` lo busca o crea). La confirmación conserva el rate limit de 60s por sucursal. Los IDs quedan sólo para URLs internas.
...
- `/admin/` y `/admin/configuracion/` → panel **propio** de la app (`admin_dashboard`, `admin_configuracion`), pensado para el usuario de matriz. Aquí se editan productos, precios, sucursales/clientes (incluyendo su correo de recordatorios), horarios de pedidos/recordatorios y la cuenta admin. El usuario `juanmanuel` pertenece al grupo `Operador de impresion`; solo entra a `/admin/` y `/admin/pedidos/<id>/imprimir/`.
...
Los reportes de aguas y sucursales comparten `latest_report_macros()`: por cada sucursal seleccionan el **macropedido confirmado** más reciente dentro de las últimas 24 horas y suman los productos de todos sus pedidos individuales activos. Un macropedido enviado no bloquea la búsqueda; se toma el siguiente confirmado anterior. No vuelvas a seleccionar directamente el `Pedido` más reciente porque eso omitiría el acumulado diario.
...
- **Frontend (UX)**: la pantalla de login muestra el estado actual de horario usando el mismo cálculo de `validar_horario_pedidos()`. `/pedidos/` ya no muestra banner de horario ni deshabilita el botón de confirmar; si el usuario intenta confirmar fuera de horario, el backend rechaza con el mensaje correspondiente.
...
`pedidos/tests.py` es la única fuente de verdad de comportamiento esperado: login por nombre de sucursal, crear/reemplazar item, agrupación diaria, máximo de cinco pedidos, impresión acumulada e individual, cambios y reversión del estado del macro, selección de reportes, ocultamiento de precios unitarios en `/pedidos/`, control de acceso admin vs. no-admin vs. operador de impresión, y el flujo completo de `/admin/configuracion/`. Antes de dar por buena cualquier modificación en `views.py`, `models.py` o `tickets.py`, corre `python manage.py test pedidos`.
...
- Hay 11 migraciones; `0011_macropedidos_diarios` crea `MacroPedido`, vincula los pedidos existentes por sucursal/fecha local y conserva totales/estados. Revisa el diff de cualquier migración nueva antes de aplicarla.
